	...
	function SlickGrid(container, data, columns, options) {
		...
		function init() {
			...
			if ($container.length < 1) {
				throw new Error(
					"SlickGrid requires a valid container that is either an HTMLELement or an ID selector (#myElement). " +
						container +
			...
			if ($container.classList) $container.classList.add(uid);
			else $container.className += " " + uid;

			if ($container.classList) $container.classList.add("ui-widget");
			else $container.className += " " + "ui-widget";
		...
		function destroy() {
			...
			$container.innerHTML = "";
			if ($container.classList) $container.classList.remove(uid);
			else
				$container.className = $container.className.replace(
					new RegExp("(^|\\b)" + uid.split(" ").join("|") + "(\\b|$)", "gi"),
					" "
				);
		}
		...
		function setSortColumns(cols) {
			sortColumns = cols;
			var numberCols =
				options.numberedMultiColumnSort && sortColumns.length > 1;
		...
		function appendCellHtml(stringArray, row, cell, colspan, item) {
			...
			addlCssClasses +=
				formatterResult && formatterResult.addClasses
		...
		function getViewportHeight() {
			return (
				parseFloat($.css($container, "height", true)) -
		...
		function navigate(dir) {
			...
			if (pos) {
				...
				scrollCellIntoView(
					pos.row,
					pos.cell,
					!isAddNewRow && options.emulatePagingWhenScrolling
				);
